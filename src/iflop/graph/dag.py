...
def topological_order(adjacency: np.ndarray) -> list[int]:
    ...
    queue: _deque[int] = _deque([node for node, degree in enumerate(indegree) if degree == 0])
    order: list[int] = []
    while queue:
        node = queue.popleft()
...
def order_index(order: _abc.Iterable[int]) -> dict[int, int]:
    return {int(node): rank for rank, node in enumerate(order)}


def prefix_nodes(order: _abc.Iterable[int], node: int) -> tuple[int, ...]:
