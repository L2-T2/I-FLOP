...
class _NodeTerms:
    ...
    pooled_scatter: np.ndarray
    n_fit: int
    per_env: list[dict[str, int]]
    effective_envs: tuple[int, ...]


class IFlopBICScorer:
    def __init__(self, dataset: MultiEnvDataset, config: IFlopScoreConfig | None = None) -> None:
        self.dataset = dataset
        self.config = config or IFlopScoreConfig()
        self.score_key = "iflop"
    ...
    def _build_node_terms(self, node: int) -> _NodeTerms:
        arrays = self._effective_arrays(node)
    ...
    def local_score(self, node: int, parents: _abc.Iterable[int]) -> float:
        ...
        return self.cache.get_or_compute(
            ...
            int(node),
            pset,
            lambda: float(_typing.cast(float, self.local_diagnostics(int(node), pset)["score"])),
        )

    def local_diagnostics(self, node: int, parents: _abc.Iterable[int]) -> dict[str, object]:
        pset = parent_tuple(parents)
        return self._diagnostics(int(node), pset)
    ...
    def _diagnostics(self, node: int, parents: tuple[int, ...]) -> dict[str, object]:
        terms = self._node_terms[int(node)]
        arrays = terms.arrays
        pooled_scatter = np.asarray(terms.pooled_scatter, dtype=float)
        n_fit = int(terms.n_fit)
        if self.config.residual_mode == "pooled_covariance":
        ...
        if self.config.residual_mode != "env_residuals":
            raise ValueError(f"unknown residual_mode: {self.config.residual_mode}")
        if parents:
