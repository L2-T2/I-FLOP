...
#[derive(Debug)]
struct IFlopError(String);

impl fmt::Display for IFlopError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}", self.0)
    }
}

impl Error for IFlopError {}

type IFlopResult<T> = Result<T, IFlopError>;
...
#[derive(Clone)]
struct IFlopSearchState {
    order: Vec<usize>,
    local_scores: Vec<IFlopNodeState>,
    total_score: f64,
}
...
impl<'a> IFlopScorer<'a> {
    fn new(dataset: &'a Dataset, config: &IFlopConfig) -> Self {
        ...
        Self {
            ...
            residual_mode: config.residual_mode.clone(),
            mode: LocalBicScoreMode::IFlopTargetFiltered,
            env_scatters,
            obs_scatter: None,
    ...
    fn new_flop_obs(dataset: &'a Dataset, config: &IFlopConfig) -> Self {
    ...
    fn new_flop_envwise(dataset: &'a Dataset, config: &IFlopConfig) -> Self {
        let env_scatters = precompute_env_scatters(dataset);
        let pooled_scatter_by_node = precompute_pooled_scatters_by_node(
            dataset,
            &env_scatters,
    ...
    fn local_score(&mut self, node: usize, parents: &[usize]) -> IFlopResult<f64> {
    ...
    fn local_state(&mut self, node: usize, parents: &[usize]) -> IFlopResult<IFlopNodeState> {
        let pset = sorted_unique(parents);
    ...
    fn local_score_uncached(&self, node: usize, parents: &[usize]) -> IFlopResult<f64> {
        Ok(self.local_state_uncached(node, parents)?.score)
    }

    fn local_state_uncached(&self, node: usize, parents: &[usize]) -> IFlopResult<IFlopNodeState> {
        ...
        if env_indices.is_empty() {
            return Err(IFlopError(format!(
                "node {} has no effective environments",
                node
            )));
        }
        if self.mode == LocalBicScoreMode::IFlopTargetFiltered
            || (self.mode == LocalBicScoreMode::FlopEnvwise
                && self.residual_mode == "pooled_covariance")
        {
            return self.local_state_iflop_shared(node, parents, &env_indices);
        }
    ...
    fn local_state_flop_obs(&self, node: usize, parents: &[usize]) -> IFlopResult<IFlopNodeState> {
        let (n, scatter) = self.obs_scatter.as_ref().ok_or_else(|| {
            IFlopError("flop_obs requires at least one observational environment".to_string())
        })?;
    ...
    fn local_state_plus(
        &mut self,
        local: &IFlopNodeState,
        parent: usize,
    ) -> IFlopResult<IFlopNodeState> {
    ...
    fn total_score(&mut self, parents: &[Vec<usize>]) -> IFlopResult<f64> {
        let mut total = 0.0;
    ...
    fn local_state_iflop_shared(
        &self,
        node: usize,
        parents: &[usize],
        env_indices: &[usize],
    ) -> IFlopResult<IFlopNodeState> {
        if self.residual_mode == "pooled_covariance" {
            let (n_fit, pooled_scatter) = self
                .pooled_scatter_by_node
                .get(node)
                .and_then(|item| item.as_ref())
                .ok_or_else(|| IFlopError(format!("missing pooled scatter for node {}", node)))?;
            ...
            let n_penalty = self.penalty_n(node, env_indices, *n_fit)?;
            let score = self.score_from_env_states(
                std::slice::from_ref(&env_state),
                n_penalty,
                parents.len(),
            );
            return Ok(IFlopNodeState {
                node,
                parents: parents.to_vec(),
                score,
                env_states: vec![env_state],
        ...
        if self.residual_mode != "env_residuals" {
            return Err(IFlopError(format!(
        ...
        let beta =
            shared_beta_from_scatter(&pooled_scatter, self.dataset.p, node, parents, self.eps);
        let mut env_states = Vec::<EnvLocalState>::with_capacity(env_indices.len());
        for idx in env_indices {
            let env = &self.dataset.envs[*idx];
            let scatter = &self.env_scatters[*idx];
    ...
    fn env_local_state_from_scratch(
        ...
        node: usize,
        parents: &[usize],
    ) -> IFlopResult<EnvLocalState> {
        let env = &self.dataset.envs[env_index];
        self.local_state_for_scatter(
            env_index,
            env.n,
            &self.env_scatters[env_index],
            node,
            parents,
        )
    }
    ...
    fn env_state_minus(
        ...
        let parents: Vec<usize> = old_parents
            .iter()
            .copied()
            .filter(|item| *item != parent)
            .collect();
    ...
    fn scatter_for_env_state<'b>(
        &'b self,
        state: &EnvLocalState,
        node: usize,
    ) -> IFlopResult<(usize, &'b [f64])> {
        if state.env_index == usize::MAX {
            if self.mode != LocalBicScoreMode::FlopObs {
                let (n, scatter) = self
                    ...
                    .and_then(|item| item.as_ref())
                    .ok_or_else(|| {
                        IFlopError(format!("missing pooled scatter for node {}", node))
                    })?;
                return Ok((*n, scatter.as_slice()));
            }
            let (n, scatter) = self.obs_scatter.as_ref().ok_or_else(|| {
                IFlopError("missing precomputed observational scatter".to_string())
            })?;
    ...
    fn score_from_env_states(
        &self,
        env_states: &[EnvLocalState],
        n_penalty: usize,
        num_parents: usize,
    ) -> f64 {
    ...
    fn penalty_n(&self, node: usize, env_indices: &[usize], n_fit: usize) -> IFlopResult<usize> {
    ...
    fn penalty_n_for_pooled_state(&self, node: usize, n_fit: usize) -> IFlopResult<usize> {
        match self.penalty_sample_mode.as_str() {
            "total" => Ok(self.dataset.total_samples()),
...
fn run_main() -> IFlopResult<()> {
    let args: Vec<String> = env::args().collect();
    if args.len() != 2 {
        return Err(IFlopError("usage: iflop <request-file>".to_string()));
    }
    let text = fs::read_to_string(&args[1]).map_err(|err| IFlopError(err.to_string()))?;
    let request = parse_request(&text)?;
    let mut result = if request.mode == "eval_order" {
        let order = request
            .order
            .clone()
            .ok_or_else(|| IFlopError("eval_order mode requires ORDER".to_string()))?;
        evaluate_order_request(&request, &order)?
    } else if request.mode == "run" {
        run_search_request(&request)?
    } else {
        return Err(IFlopError(format!("unknown mode {}", request.mode)));
    };
...
fn apply_output_semantics(request: &Request, candidate: &mut Candidate) {
    if request.score_key == "iflop" {
        candidate.adjacency = icpdag_adjacency(&candidate.dag_adjacency, &request.dataset);
        candidate.adjacency_type = "i_cpdag".to_string();
    }
}

fn evaluate_order_request(request: &Request, order: &[usize]) -> IFlopResult<Candidate> {
    validate_order(order, request.dataset.p)?;
    match request.score_key.as_str() {
        "iflop" => {
            let mut scorer = IFlopScorer::new(&request.dataset, &request.iflop);
            let parents = parent_sets_for_order_iflop(&mut scorer, order, request.search.atol)?;
        ...
        "flop_obs" => {
            let mut scorer = IFlopScorer::new_flop_obs(&request.dataset, &request.iflop);
            let parents = parent_sets_for_order_iflop(&mut scorer, order, request.search.atol)?;
            let dag_adjacency = adjacency_from_parents(&parents, request.dataset.p);
            let adjacency = cpdag_adjacency_from_parents(&parents, request.dataset.p);
            let score = scorer.total_score(&parents)?;
        ...
        "flop_envwise" => {
            let mut scorer = IFlopScorer::new_flop_envwise(&request.dataset, &request.iflop);
            let parents = parent_sets_for_order_iflop(&mut scorer, order, request.search.atol)?;
            let dag_adjacency = adjacency_from_parents(&parents, request.dataset.p);
            let adjacency = cpdag_adjacency_from_parents(&parents, request.dataset.p);
            let score = scorer.total_score(&parents)?;
        ...
        other => Err(IFlopError(format!(
            "Rust backend currently supports flop_obs, flop_envwise and iflop, got {}",
            other
        ))),
    }
}

fn run_search_request(request: &Request) -> IFlopResult<Candidate> {
    match request.score_key.as_str() {
        "iflop" => {
            let mut scorer = IFlopScorer::new(&request.dataset, &request.iflop);
...
fn parse_request(text: &str) -> IFlopResult<Request> {
    let mut lines = text.lines();
    let header = lines
        .next()
        .ok_or_else(|| IFlopError("empty Rust request".to_string()))?
        .trim();
    if header != "IFLOP_V1" {
        return Err(IFlopError(format!("bad request header {}", header)));
    }

    let mut mode = "run".to_string();
    let mut score_key = "iflop".to_string();
    ...
    let mut iflop = IFlopConfig {
        eps: 1.0e-8,
        penalty_sample_mode: "total".to_string(),
        fit_weight: 1.0,
        penalty_weight: 1.0,
        residual_mode: "pooled_covariance".to_string(),
    };
    ...
    while let Some(raw) = lines.next() {
        ...
        match parts[0] {
            ...
            "EPS" => iflop.eps = parse_f64(expect_value(&parts, "EPS")?, "EPS")?,
            "PENALTY_SAMPLE_MODE" => {
                iflop.penalty_sample_mode = expect_value(&parts, "PENALTY_SAMPLE_MODE")?.to_string()
            }
            "SCORE_FIT_WEIGHT" => {
                iflop.fit_weight = parse_f64(
                    expect_value(&parts, "SCORE_FIT_WEIGHT")?,
                    "SCORE_FIT_WEIGHT",
                )?
            }
            "SCORE_PENALTY_WEIGHT" => {
                iflop.penalty_weight = parse_f64(
                    expect_value(&parts, "SCORE_PENALTY_WEIGHT")?,
                    "SCORE_PENALTY_WEIGHT",
                )?
            }
            "RESIDUAL_MODE" => {
                iflop.residual_mode = expect_value(&parts, "RESIDUAL_MODE")?.to_string()
            }
            "SEARCH_ILS" => {
            ...
            "ORDER" => {
                if parts.len() < 2 {
                    return Err(IFlopError("ORDER requires length".to_string()));
                }
                let len = parse_usize(parts[1], "ORDER length")?;
                if parts.len() != len + 2 {
                    return Err(IFlopError("ORDER length does not match values".to_string()));
                }
            ...
            "ENV" => {
                if parts.len() < 4 {
                    return Err(IFlopError("ENV requires id n target_count".to_string()));
                }
                ...
                if parts.len() != target_count + 4 {
                    return Err(IFlopError(
                        "ENV target_count does not match values".to_string(),
                    ));
                }
                let p_value =
                    p.ok_or_else(|| IFlopError("P must appear before ENV".to_string()))?;
                ...
                for row_idx in 0..n {
                    let row_line = lines.next().ok_or_else(|| {
                        IFlopError(format!("ENV {} missing row {}", env_id, row_idx))
                    })?;
            ...
            "END" => break,
            other => return Err(IFlopError(format!("unknown request field {}", other))),
        }
    }

    let p_value = p.ok_or_else(|| IFlopError("P is required".to_string()))?;
    if envs.is_empty() {
        return Err(IFlopError("at least one ENV is required".to_string()));
    }
    for env in &envs {
        if env.p != p_value {
            return Err(IFlopError(format!("ENV {} has inconsistent P", env.id)));
        }
...
fn run_iflop_order_search(
    scorer: &mut IFlopScorer<'_>,
    config: &SearchConfig,
) -> IFlopResult<Candidate> {
    let p = scorer.dataset.p;
    let mut rng = SimpleRng::new(config.random_seed);
    let mut best_order = resolve_iflop_initial_order(scorer.dataset, &mut rng);
    let mut best_state: Option<IFlopSearchState> = None;
    ...
    for iteration in 0..=config.ils_restarts {
        ...
        let mut state = perm_to_iflop_state(scorer, &order, config.atol)?;
        local_search_iflop(scorer, &mut state, config)?;
        state = perm_to_iflop_state(scorer, &state.order, config.atol)?;
    ...
    best_state
        .map(|state| candidate_from_iflop_state(&state, scorer.dataset.p))
        .ok_or_else(|| IFlopError("search produced no candidate".to_string()))
}

fn parent_sets_for_order_iflop(
    scorer: &mut IFlopScorer<'_>,
    order: &[usize],
    atol: f64,
) -> IFlopResult<Vec<Vec<usize>>> {
    let mut parents = vec![Vec::<usize>::new(); scorer.dataset.p];
    for pos in 0..order.len() {
        let node = order[pos];
        parents[node] = fit_parents_iflop(scorer, node, &order[..pos], atol)?.parents;
    }
    Ok(parents)
}

fn local_iflop_result(
    scorer: &mut IFlopScorer<'_>,
    node: usize,
    parents: &[usize],
) -> IFlopResult<IFlopNodeState> {
...
fn grow_iflop(
    ...
    loop {
        ...
        for candidate in non_parents.clone().iter().rev() {
            let new_local = iflop_local_score_plus(scorer, local, *candidate)?;
...
fn shrink_iflop(
    ...
    loop {
        ...
        for parent in local.parents.clone().iter().rev() {
            let new_local = iflop_local_score_minus(scorer, local, *parent)?;
            if new_local.score <= local.score + atol {
                *local = new_local;
...
fn fit_parents_iflop(
    scorer: &mut IFlopScorer<'_>,
    node: usize,
    prefix: &[usize],
    atol: f64,
) -> IFlopResult<IFlopNodeState> {
    let mut local = local_iflop_result(scorer, node, &[])?;
    let mut non_parents: Vec<usize> = prefix
        .iter()
        .copied()
    ...
    loop {
        ...
        grow_iflop(scorer, &mut local, &mut non_parents, atol)?;
        shrink_iflop(scorer, &mut local, &mut non_parents, atol)?;
...
fn fit_parents_iflop_plus(
    scorer: &mut IFlopScorer<'_>,
    mut local: IFlopNodeState,
    prefix: &[usize],
    new_parents: &[usize],
    atol: f64,
) -> IFlopResult<IFlopNodeState> {
    for parent in new_parents {
        if !local.parents.contains(parent) {
            local = iflop_local_score_plus(scorer, &local, *parent)?;
        }
    }
...
fn fit_parents_iflop_minus(
    ...
    local = iflop_local_score_minus(scorer, &local, remove_parent)?;
    let mut non_parents = set_minus(prefix, &local.parents);
    loop {
        let old_score = local.score;
        grow_iflop(scorer, &mut local, &mut non_parents, atol)?;
...
fn perm_to_iflop_state(
    scorer: &mut IFlopScorer<'_>,
    order: &[usize],
    atol: f64,
) -> IFlopResult<IFlopSearchState> {
    let mut local_scores = (0..order.len())
        .map(|node| IFlopNodeState {
            node,
            parents: Vec::new(),
            score: f64::INFINITY,
            env_states: Vec::new(),
    ...
    for idx in 0..order.len() {
        let node = order[idx];
        local_scores[node] = fit_parents_iflop(scorer, node, &order[..idx], atol)?;
    }
...
fn reinsert_iflop(
    ...
    let v_index = state
        ...
        .ok_or_else(|| IFlopError(format!("node {} not found in current order", node)))?;
    ...
    let mut v_best_local: Vec<Option<IFlopNodeState>> = vec![None; p];
    let mut z_best_local: Vec<Option<IFlopNodeState>> = vec![None; p];

    for pos in (0..v_index).rev() {
        let z = state.order[pos];
        let mut prefix = state.order[0..pos].to_vec();
        let old_v_score = v_curr_local.score;
        let v_new_local = fit_parents_iflop_minus(scorer, v_curr_local, &prefix, z, atol)?;
        let v_score_diff = v_new_local.score - old_v_score;
        v_curr_local = v_new_local.clone();
        prefix.push(node);
        let z_curr_local = state.local_scores[z].clone();
        let z_new_local =
            fit_parents_iflop_plus(scorer, z_curr_local.clone(), &prefix, &[node], atol)?;
    ...
    for pos in (v_index + 1)..p {
        ...
        let old_v_score = v_curr_local.score;
        let v_new_local = fit_parents_iflop_plus(scorer, v_curr_local, &prefix, &[z], atol)?;
        let v_score_diff = v_new_local.score - old_v_score;
        v_curr_local = v_new_local.clone();
        remove_first_usize(&mut prefix, z);
        let z_curr_local = state.local_scores[z].clone();
        let z_new_local =
            fit_parents_iflop_minus(scorer, z_curr_local.clone(), &prefix, node, atol)?;
        let z_score_diff = z_new_local.score - z_curr_local.score;
    ...
    state.local_scores[node] = v_best_local[best_ins_pos].clone().ok_or_else(|| {
        IFlopError("missing stored best local score for reinserted node".to_string())
    })?;
    if best_ins_pos < v_index {
        for (offset, z) in state.order[best_ins_pos..v_index]
            .iter()
            .copied()
            .enumerate()
        {
            state.local_scores[z] = z_best_local[best_ins_pos + offset]
                .clone()
                .ok_or_else(|| IFlopError(format!("missing updated local score for node {}", z)))?;
        }
    } else {
...
fn local_search_iflop(
    ...
    loop {
        ...
        for node in state.order.clone() {
            reinsert_iflop(scorer, state, node, config.atol)?;
        }
        sweep_count += 1;
...
fn candidate_from_iflop_state(state: &IFlopSearchState, p: usize) -> Candidate {
...
fn resolve_iflop_initial_order(dataset: &Dataset, rng: &mut SimpleRng) -> Vec<usize> {
    let obs_indices = dataset.observational_env_indices();
...
fn pivoted_cholesky_order(corr: &[f64], p: usize) -> Option<Vec<usize>> {
    ...
    let second_index = if first_pair.1 == 0 {
        first_pair.0
    } else {
        first_pair.1
    };
...
fn validate_order(order: &[usize], p: usize) -> IFlopResult<()> {
    if order.len() != p {
        return Err(IFlopError(format!(
    ...
    if sorted != expected {
        return Err(IFlopError(
            "order is not a permutation of variables".to_string(),
        ));
    }
    Ok(())
}

fn expect_value<'a>(parts: &'a [&str], field: &str) -> IFlopResult<&'a str> {
    if parts.len() != 2 {
        return Err(IFlopError(format!("{} requires exactly one value", field)));
    }
    Ok(parts[1])
}

fn parse_usize(value: &str, field: &str) -> IFlopResult<usize> {
    value
        .parse::<usize>()
        .map_err(|err| IFlopError(format!("failed to parse {} as usize: {}", field, err)))
}

fn parse_isize(value: &str, field: &str) -> IFlopResult<isize> {
    value
        .parse::<isize>()
        .map_err(|err| IFlopError(format!("failed to parse {} as isize: {}", field, err)))
}

fn parse_u64(value: &str, field: &str) -> IFlopResult<u64> {
    value
        .parse::<u64>()
        .map_err(|err| IFlopError(format!("failed to parse {} as u64: {}", field, err)))
}

fn parse_i32(value: &str, field: &str) -> IFlopResult<i32> {
    value
        .parse::<i32>()
        .map_err(|err| IFlopError(format!("failed to parse {} as i32: {}", field, err)))
}

fn parse_f64(value: &str, field: &str) -> IFlopResult<f64> {
    value
        .parse::<f64>()
        .map_err(|err| IFlopError(format!("failed to parse {} as f64: {}", field, err)))
}
...
mod tests {
    ...
    fn test_pivoted_cholesky_failure_falls_back_to_random_permutation() {
        ...
        let actual = resolve_iflop_initial_order(&dataset, &mut actual_rng);
