...
def parent_tuple(parents: Iterable[int]) -> tuple[int, ...]:
...
def residual_variance_ols(data: np.ndarray, node: int, parents: Iterable[int], eps: float) -> tuple[float, float]:
    """Return residual variance and RSS using an intercept."""
...
def residual_variance_scatter(
    ...
    """Return conditional variance from a centered scatter matrix."""
...
def residual_variance_cholesky(
    ...
    """Return conditional variance using a Cholesky solve when possible."""
...
def bic_cost_terms(
    ...
    *,
    ...
    """Return total, likelihood cost, and BIC penalty for minimization."""
