...
from iflop_final.graph.dag import as_square_adjacency, parents_from_adjacency, topological_order
...
def dag_to_cpdag_proxy(adjacency: np.ndarray) -> np.ndarray:
    """Backward-compatible alias for the exact CPDAG conversion."""

    return dag_to_cpdag(adjacency)


def dag_to_icpdag(
    adjacency: np.ndarray,
    intervention_targets: Mapping[int, set[int]] | None = None,
) -> np.ndarray:
